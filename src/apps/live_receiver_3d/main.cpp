...
struct CLIOptions {
    ...
    OutputLatencyMode outputLatencyMode = OutputLatencyMode::High;
...
struct QueuedFrame3D {
    SocketFrame3D frame;
    double interpolationHintMs = DEFAULT_FRAME_INTERVAL_MS;
};

class IncomingFrameQueue {
public:
    struct Snapshot {
        uint64_t pushed = 0;
        uint64_t popped = 0;
    ...
    bool tryPopLatest(QueuedFrame3D& frame) {
        std::unique_lock<std::mutex> lock(mutex_, std::try_to_lock);
        if (!lock.owns_lock() || !latestFrame_.has_value()) {
            return false;
        }
    ...
    Snapshot snapshot() const {
        ...
        return Snapshot{
            ...
            popped_,
            overwritten_,
            latestFrame_.has_value(),
        };
    }

private:
    mutable std::mutex mutex_;
    std::optional<QueuedFrame3D> latestFrame_;
    uint64_t pushed_ = 0;
    uint64_t popped_ = 0;
...
void printUsage(const char* executableName) {
    ...
    std::cout << "  --stream-timeout-ms <ms>    ZeroMQ receive timeout only (default: 34)\n";
    std::cout << "  --output-latency-mode <low|high>  Output latency target (default: high)\n";
    std::cout << "  --audio-buffer-ms <ms>      Explicit output latency buffer in milliseconds\n";
...
bool parseOutputLatencyModeArg(std::string_view text, OutputLatencyMode& outMode) {
    if (text == "low") {
        outMode = OutputLatencyMode::Low;
        return true;
    }
    if (text == "high") {
        outMode = OutputLatencyMode::High;
        return true;
    }
    return false;
}
...
CLIOptions parseCommandLine(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--output-latency-mode") {
            const char* value = requireValue("--output-latency-mode");
            OutputLatencyMode parsedMode = options.outputLatencyMode;
            if (value == nullptr || !parseOutputLatencyModeArg(value, parsedMode)) {
                std::cerr << fmt::format(
                    "Invalid --output-latency-mode value: {} (expected low or high)\n",
                    value != nullptr ? value : "<null>");
                options.valid = false;
            } else {
                options.outputLatencyMode = parsedMode;
            }
            continue;
        }
        if (arg == "--audio-buffer-ms") {
            const char* value = requireValue("--audio-buffer-ms");
            if (value != nullptr && !parseFloatArg(value, options.audioBufferMs)) {
                std::cerr << fmt::format("Invalid --audio-buffer-ms value: {}\n", value);
                options.valid = false;
            }
            continue;
        }
    ...
    if (options.audioBufferMs < 0.0f) {
        std::cerr << "--audio-buffer-ms must be >= 0\n";
        options.valid = false;
    }
...
bool initializeSteamAudioResources(bool useDefaultHRTF,
    ...
    resources.distanceModel.minDistance = 0.1f;
...
struct AudioRenderState {
    ...
    int interpolationTotalSamples = 1;
    int interpolationConsumedSamples = 1;
    bool hasLastCallbackAt = false;
    std::chrono::steady_clock::time_point lastCallbackAt{};

    std::atomic<uint64_t> outputUnderflowCount{0};
    std::atomic<uint64_t> callbackCount{0};
    std::atomic<uint64_t> callbackRenderFailures{0};
    std::atomic<uint64_t> callbackJitterSampleCount{0};
    std::atomic<uint64_t> callbackJitterAbsSumUs{0};
    std::atomic<uint64_t> callbackJitterAbsMaxUs{0};
    std::atomic<uint64_t> queueMissCount{0};
    std::atomic<size_t> activeObjectCount{0};
    std::atomic<size_t> activeVoiceCount{0};
    std::atomic<int> latestFrameNumber{-1};
    std::atomic<size_t> latestFrameObjectCount{0};
};
...
bool renderAndFillOutput(const ObjectTracker3D& tracker,
                         ...
                         SteamAudioResources& resources,
                         std::unordered_map<int, SourceVoiceState>& voices,
                         int& interpolationConsumedSamples,
                         int interpolationTotalSamples,
                         float* outputInterleaved,
                         size_t& activeObjectsOut) {
    activeObjectsOut = 0;
    if (samplesToRender <= 0 || outputInterleaved == nullptr) {
        return true;
    }

    const int safeInterpolationTotal = std::max(1, interpolationTotalSamples);
    int rendered = 0;

    while (rendered < samplesToRender && gRunning.load()) {
        const int chunkSamples = std::min(FRAME_SIZE, samplesToRender - rendered);
        interpolationConsumedSamples =
            std::min(safeInterpolationTotal, interpolationConsumedSamples + chunkSamples);
        const float interpolationFactor =
            static_cast<float>(interpolationConsumedSamples) /
            static_cast<float>(safeInterpolationTotal);

        const auto activeObjects = tracker.getInterpolatedActiveObjects(interpolationFactor);
        activeObjectsOut = activeObjects.size();
        ...
        for (int i = 0; i < chunkSamples; ++i) {
            ...
            outputInterleaved[(rendered + i) * 2] = left;
            outputInterleaved[(rendered + i) * 2 + 1] = right;
        ...
        if (peak > runtimeConfig.limiterThreshold && peak > 0.0f) {
            ...
            for (int i = 0; i < chunkSamples; ++i) {
                outputInterleaved[(rendered + i) * 2] *= limiterGain;
                outputInterleaved[(rendered + i) * 2 + 1] *= limiterGain;
            }
        }
...
int portAudioCallback(const void* input,
    ...
    auto* state = static_cast<AudioRenderState*>(userData);
    auto* outputInterleaved = static_cast<float*>(output);
    const size_t sampleCount = static_cast<size_t>(frameCount) * 2;
    if (outputInterleaved != nullptr) {
        std::fill(outputInterleaved, outputInterleaved + sampleCount, 0.0f);
    }

    if (state == nullptr || outputInterleaved == nullptr || frameCount == 0) {
        return paContinue;
    }

    state->callbackCount.fetch_add(1, std::memory_order_relaxed);
    if ((statusFlags & paOutputUnderflow) != 0u) {
        state->outputUnderflowCount.fetch_add(1, std::memory_order_relaxed);
    }

    const auto now = std::chrono::steady_clock::now();
    if (state->hasLastCallbackAt) {
        const uint64_t actualUs = static_cast<uint64_t>(
            std::llround(std::chrono::duration_cast<std::chrono::duration<double, std::micro>>(
                             now - state->lastCallbackAt)
                             .count()));
        const uint64_t expectedUs = static_cast<uint64_t>(
            std::llround((static_cast<double>(frameCount) * 1000000.0) /
                         static_cast<double>(std::max(1, state->runtimeConfig.sampleRate))));
        const uint64_t jitterAbsUs = (actualUs > expectedUs)
                                         ? (actualUs - expectedUs)
                                         : (expectedUs - actualUs);
        state->callbackJitterSampleCount.fetch_add(1, std::memory_order_relaxed);
        state->callbackJitterAbsSumUs.fetch_add(jitterAbsUs, std::memory_order_relaxed);
        updateAtomicMax(state->callbackJitterAbsMaxUs, jitterAbsUs);
    }
    ...
    const uint64_t renderAdvanceUs = static_cast<uint64_t>(
        std::llround((static_cast<double>(frameCount) * 1000000.0) /
                     static_cast<double>(std::max(1, state->runtimeConfig.sampleRate))));
    state->renderTimelineUs += renderAdvanceUs;

    QueuedFrame3D queuedFrame;
    const bool hasFrame = state->incomingFrames->tryPopLatest(queuedFrame);
    if (hasFrame) {
        double interpolationHintMs = queuedFrame.interpolationHintMs;
        if (!std::isfinite(interpolationHintMs) || interpolationHintMs <= 0.0) {
            interpolationHintMs = DEFAULT_FRAME_INTERVAL_MS;
        }
        const double usedInterpolationMs = std::max(
            1.0,
            std::min(static_cast<double>(state->runtimeConfig.maxInterpWindowMs),
                     interpolationHintMs));
        state->interpolationTotalSamples = std::max(
            1,
            static_cast<int>(std::llround((usedInterpolationMs / 1000.0) *
                                          static_cast<double>(std::max(1, state->runtimeConfig.sampleRate)))));
        state->interpolationConsumedSamples = 0;

        state->tracker->updateFromFrame(queuedFrame.frame,
                                        state->renderTimelineUs,
                                        static_cast<float>(frameCount));
        state->latestFrameNumber.store(queuedFrame.frame.frame_number, std::memory_order_relaxed);
        state->latestFrameObjectCount.store(queuedFrame.frame.objects.size(), std::memory_order_relaxed);
    } else {
        state->queueMissCount.fetch_add(1, std::memory_order_relaxed);
        state->tracker->updateWithoutFrame(state->renderTimelineUs, static_cast<float>(frameCount));
    }

    const std::vector<int> releasable = state->tracker->collectReleasableObjects(state->renderTimelineUs);
    for (const int objectId : releasable) {
        releaseVoiceForObject(objectId, state->voices);
    }

    size_t activeObjects = 0;
    if (!renderAndFillOutput(*state->tracker,
                             ...
                             *state->resources,
                             state->voices,
                             state->interpolationConsumedSamples,
                             state->interpolationTotalSamples,
                             outputInterleaved,
                             activeObjects)) {
        state->callbackRenderFailures.fetch_add(1, std::memory_order_relaxed);
    }
...
bool openOutputStream(int requestedDeviceIndex,
                      int requestedSampleRate,
                      const RuntimeAudioConfig& runtimeConfig,
                      AudioRenderState& renderState,
                      PaStream*& stream,
                      int& actualSampleRate,
                      double& selectedLatencyMs) {
    stream = nullptr;
    actualSampleRate = FALLBACK_SAMPLE_RATE;
    selectedLatencyMs = 0.0;
    ...
    const int deviceDefaultSampleRate =
        static_cast<int>(std::llround(deviceInfo->defaultSampleRate));
    ...
    const double defaultLatencySec =
        (runtimeConfig.outputLatencyMode == OutputLatencyMode::Low)
            ? deviceInfo->defaultLowOutputLatency
            : deviceInfo->defaultHighOutputLatency;
    const double selectedLatencySec =
        (runtimeConfig.audioBufferMs > 0.0f)
            ? (static_cast<double>(runtimeConfig.audioBufferMs) / 1000.0)
            : defaultLatencySec;
    selectedLatencyMs = selectedLatencySec * 1000.0;

    renderState.runtimeConfig = runtimeConfig;
    renderState.runtimeConfig.sampleRate = selectedSampleRate;
    renderState.tracker.emplace(selectedSampleRate,
                                runtimeConfig.noFrameFadeMs,
                                runtimeConfig.holdLastPositionMs);
    renderState.renderTimelineUs = 0;
    renderState.audioTimeSec = 0.0;
    renderState.interpolationTotalSamples = 1;
    renderState.interpolationConsumedSamples = 1;
    renderState.voices.clear();
    renderState.hasLastCallbackAt = false;

    PaStreamParameters outputParameters{};
    outputParameters.device = outputDevice;
    outputParameters.channelCount = 2;
    outputParameters.sampleFormat = paFloat32;
    outputParameters.suggestedLatency = selectedLatencySec;
    outputParameters.hostApiSpecificStreamInfo = nullptr;

    unsigned long framesPerBuffer = paFramesPerBufferUnspecified;
    if (runtimeConfig.audioBufferMs > 0.0f) {
        const long long requestedFrames = std::llround(
            selectedLatencySec * static_cast<double>(selectedSampleRate));
        framesPerBuffer =
            static_cast<unsigned long>(std::max<long long>(64, requestedFrames));
    }
...
bool startOutputStream(PaStream* stream) {
    if (stream == nullptr) {
        return false;
    }
...
int main(int argc, char* argv[]) {
    ...
    runtimeConfig.outputLatencyMode = options.outputLatencyMode;
    runtimeConfig.audioBufferMs = options.audioBufferMs;
    ...
    double selectedLatencyMs = 0.0;
    if (!openOutputStream(options.deviceIndex,
                          options.sampleRate,
                          runtimeConfig,
                          renderState,
                          stream,
    ...
    const double actualLatencyMs =
        (streamInfo != nullptr) ? (streamInfo->outputLatency * 1000.0) : selectedLatencyMs;
    const char* latencyModeLabel =
        (runtimeConfig.outputLatencyMode == OutputLatencyMode::High) ? "high" : "low";
    if (runtimeConfig.audioBufferMs > 0.0f) {
        std::cout << fmt::format(
            "Output latency mode: {} (explicit --audio-buffer-ms={:.1f}), actual stream latency: {:.1f} ms\n",
            latencyModeLabel,
            runtimeConfig.audioBufferMs,
            actualLatencyMs);
    } else {
        std::cout << fmt::format(
            "Output latency mode: {}, target latency: {:.1f} ms, actual stream latency: {:.1f} ms\n",
            latencyModeLabel,
            selectedLatencyMs,
            actualLatencyMs);
    }
    ...
    std::cout << fmt::format(
        "Total PortAudio output underflows: {}\n",
        renderState.outputUnderflowCount.load(std::memory_order_relaxed));
